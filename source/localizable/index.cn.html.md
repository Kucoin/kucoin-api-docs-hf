...
請求參數 | 類型 | 含義            
--------- | ---------|---------------
timeout | Integer | 自動撤單觸發設置時間，單位 秒。範圍:timeout = -1（表示取消設置）, 或者  5 <= timeout <= 86400，當該參數改變時，會覆蓋之前的設置。
symbols | String |[可選] 交易對列表。此參數不為空時，逗號隔開，最多支持50個交易對。為空表示所有交易對。 當該參數改變時，會覆蓋之前的設置。
